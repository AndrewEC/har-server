...
class RouteMapTest(unittest.TestCase):
    ...
    def test_find_entry_for_request(self,
        ...
        mock_request_rewriter.apply_browser_request_rewrite_rules = MagicMock(return_value=rewritten_incoming_request)
        mock_request_rewriter.apply_entry_request_rewrite_rules = MagicMock(return_value=rewritten_entry_request)

        mock_exclusion_filter.should_exclude_entry = MagicMock(return_value=False)

        har_entry = Mock(request=Mock())
        mock_har_parser.get_har_file_contents = MagicMock(return_value=[Mock(entries=[har_entry])])
